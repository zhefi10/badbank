...
function CreateAccount() {
    ...
    function validate (values) {
        ...
        if (!values.password) {
          ...
          disableBtn = true;
            ...
            errors.password = 'Password length must have more than 8 characters or longer';
            disableBtn = true;
        };
    ...
    return (
        <Card 
            ...
            text={show ? <>Register here to start hating the benefits of BadBank</>:<>Click <span className="badge bg-success">You really want add another account?</span> <br/> or <br/> <span className="badge bg-success">Login</span> to access to your account</>}
            body={
                <form onSubmit={formik.handleSubmit}>
                    ...
                    Password<br/>
                    <input type="password" autoComplete="actual-password" className="form-control" id="password" placeholder="Enter a password" value={formik.values.password} onChange={formik.handleChange} onBlur={formik.handleBlur}/>{formik.touched.password && formik.errors.password ? (<div id="pswError" style={{color: 'red'}}>{formik.errors.password}</div>) : null}<br/>
